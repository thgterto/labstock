    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500">Manage your chemical and equipment definitions.</p>
        </div>
        <button 
      ...
      <div className="flex gap-4">
        <div className="relative flex-1 max-w-md">
          ...
          <input 
            type="text" 
            placeholder="Search by name, CAS, or code..." 
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold text-slate-700 text-right">Total Stock</th>
              ...
              {filteredItems.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="font-medium text-slate-900">{item.totalQuantity} <span className="text-slate-500 text-xs font-normal">Units/L</span></div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="text-lg font-semibold text-slate-900">Add New Catalog Item</h3>
              ...
              {newItem.category === 'CHEMICAL' && (
                <div className="p-4 bg-slate-50 rounded-lg border border-slate-100 space-y-4">
                   <div className="flex items-center gap-2 mb-2">
                     ...
                     <h4 className="text-sm font-semibold text-slate-700">Chemical Properties</h4>
                   </div>
                   <div className="grid grid-cols-2 gap-4">
                     <div>
                       <label className="block text-xs font-medium text-slate-500 mb-1">CAS Number</label>
                       <input 
                        type="text" 
                   ...
                   <div>
                      <label className="block text-xs font-medium text-slate-500 mb-1">Detected Hazards (GHS)</label>
                      <div className="flex flex-wrap gap-2 min-h-[32px] p-2 bg-white border border-slate-200 rounded">
                        ...
                        ) : <span className="text-xs text-slate-400 italic">No hazards detected</span>}
                      </div>
                   </div>
                </div>
              )}
